...
namespace Inferno.InfernoScripts.World
{
    internal class ChaosHeli : InfernoScript
    {
            ...
            while (IsActive && !ct.IsCancellationRequested)
            {
                ...
                if (elapsedTime > 40)
                {
                    ...
                    if ((PlayerPed.IsSafeExist() && !_heli.IsSafeExist()) || _heli.IsDead ||
                        !_heli.IsInRangeOfIgnoreZ(PlayerPed.Position, 400.0f))
                    {
                        ResetHeli();
                        return;
                    }
                ...
                if (_isDebug)
                {
                    DrawTextL($"[Debug] Distance: {_heli.Position.DistanceTo2D(PlayerPed.Position):F1} [m]");
                }
        ...
        private void MoveHeli(Ped heliDriver, Vector3 targetPosition)
        {
            ...
            if (_heli.IsInRangeOfIgnoreZ(targetPosition, 50))
            {
                //プレイヤに近い場合は攻撃する
            ...
            else
            {
                if (_isNearPlayer)
                {
                    ...
                    if (_isDebug)
                    {
                        DrawText("DEBUG: Follow player.", 3);
                    }
                }

                _heli.DriveTo(_heliDriver, targetPosition, 100, DrivingStyle.Normal);
            }
        }
        ...
        private void CreateChaosHeli()
        {
            if (_isDebug)
            {
                DrawText("DEBUG: Create new chaos heli.", 3);
            }
            ...
            try
            {
                ...
                var targetBase = playerPosition.Around(100);
                var targetGround = GTA.World.GetGroundHeight(targetBase);


                var spawnHeliPosition = new Vector3(targetBase.X, targetBase.Y, targetGround + 200);
        ...
        public override string DisplayName => ChaosHeliLocalize.Name;

        public override string Description => ChaosHeliLocalize.Description;
